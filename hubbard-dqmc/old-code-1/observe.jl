...
function observe()
    if double_check
        ...
        open(error_path, "a") do error_file
            println(error_file, relative_err(G_up(τ_now), G_up))
            println(error_file, relative_err(G_down(τ_now), G_dn))
        end
    end
    ...
    open(observables_path, "a") do observable_file
        ...
        mag = sum(Iterators.product(1 : n_sites, 1 : n_sites)) do point_pair
            i, j = point_pair
            r_i = site_to_coord(i) |> collect
            r_j = site_to_coord(j) |> collect
